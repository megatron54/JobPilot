...
from app.core.security import safe_join, safe_slug
...
def save_job_offer(job_data: dict) -> dict:
    ...
    # Generate filename from company and position (sanitized against path traversal)
    company = safe_slug(job_data.get("company", ""), default="unknown")
    position = safe_slug(job_data.get("position", ""), default="unknown")
    ...
    filepath = safe_join(jobs_dir, filename)
...
def get_job(job_id: str) -> dict:
    ...
    jobs_dir = Path(settings.jobs_dir)
    try:
        filepath = safe_join(jobs_dir, f"{job_id}.json")
    except ValueError:
        raise FileNotFoundError(f"Job not found: {job_id}") from None
